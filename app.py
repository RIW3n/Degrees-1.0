...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.pushButton_16.clicked.connect(self.plot_robot_position)
    ...
    def serial_communication(self):
        try:
            while True:
                self.odom = self.serial.readline().decode('latin1').strip()
                try:
                    self.tempo = self.odom.split(",")
                    self.x = float(self.tempo[0])
                    self.y = float(self.tempo[1])
                    self.t = float(self.tempo[2])
                    self.data_queue.put((self.x, self.y, self.t))
